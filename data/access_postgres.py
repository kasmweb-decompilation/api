...
# Embedded file name: data/access_postgres.py
import os, typing, uuid, select, hashlib, datetime, threading, logging, logging.config, random, string
try:
    ...
    from data.model import MODEL_CLASSES, MODEL_CLASS_ORDER, IMPORT_EXPORT_IGNORE, User, Image, Kasm, Server, ImageAttribute, UserAttributes, Model, ConfigSetting, Newsletter, GroupSetting, Group, UserGroup, GroupImage, LDAPConfig, SAMLConfig, Installation, License, Manager, Zone, KasmAttribute, Log, Api, Accounting, Company, Domain, FilterPolicy, Cart, BrandingConfig, SessionToken, StagingConfig, CastConfig, SessionPermission, OIDCConfig, SSOToGroupMapping, ConnectionProxy, ServerPool, AzureVMConfig, AzureDNSConfig, AutoScaleConfig, ServerPool, AwsVMConfig, AwsDNSConfig, DigitalOceanVMConfig, DigitalOceanDNSConfig, OracleVMConfig, OracleDNSConfig, GcpVMConfig, GcpDNSConfig, VsphereVMConfig, PhysicalToken, SSOAttributeToUserFieldMapping, FileMap, Registry, StorageMapping, StorageProvider, Schedule, WebauthnCredential, WebauthnRequest, SessionRecording, GroupPermission, OpenStackVMConfig
    ...
    class DataAccessPostgres(DataAccessBase):
        ...
        def __init__(self, config=None):
            if DataAccessPostgres.db_engine is None:
                DataAccessPostgres.engine = create_engine((self.get_url(config)), pool_pre_ping=True,
                ...
                for table in [table for table in tables if table not in IMPORT_EXPORT_IGNORE]:
                    ...
                    for col in obj.__table__.columns:
                        try:
                            ...
                            if col.nullable:
                                _required = False
                            elif col.default or col.server_default:
                                _required = False
                            else:
                                _required = True
                    ...
                    for record in import_data[table]:
                        for r in required_or_fk:
                            if record.get(r) is not None:
                                if r in required_or_fk_checked:
            ...
            try:
                ...
                for table in [table for table in MODEL_CLASS_ORDER if table not in IMPORT_EXPORT_IGNORE]:
                    if table in tables_todo:
                        ...
                        for record in data:
                            if table == "zones":
                                if "primary_manager_id" in record:
        ...
        def validateSessionTokenParse error at or near `LOAD_CONST' instruction at offset 132
        ...
        def setConfigSetting(self, category, name, title, value, value_type, services_restart=None, description=None, sanitize=False):
            ...
            try:
                try:
                    ...
                    if conf is None:
                        conf = ConfigSetting(name=name, value=value,
                          title=title,
        ...
        def image_on_agentParse error at or near `POP_BLOCK' instruction at offset 100
        ...
        def create_update_image(self, image_id=None, image_name=None, cores=None, description=None, docker_registry=None, docker_token=None, docker_user=None, enabled=None, friendly_name=None, hash=None, memory=None, name=None, x_res=None, y_res=None, run_config=None, restrict_to_network=None, restrict_network_names=None, restrict_to_server=None, server_id=None, persistent_profile_path=None, volume_mappings=None, image_src=None, exec_config=None, restrict_to_zone=None, zone_id=None, filter_policy_id=None, filter_policy_force_disabled=None, session_time_limit=None, categories=None, allow_network_selection=None, require_gpu=None, gpu_count=None, hidden=None, notes=None, image_type=None, server_pool_id=None, link_url=None, cpu_allocation_method=None, uncompressed_size_mb=None, launch_config=None):
            ...
            if docker_token:
                if not is_sanitized(docker_token):
            ...
            if name:
                ...
                elif restrict_to_zone == False:
                    image.restrict_to_zone = False
                if zone_id:
                    image.zone_id = zone_id
                if filter_policy_id != None:
                    if filter_policy_id == "":
                ...
                if image.persistent_profile_path:
                    if image.persistent_profile_path.startswith("s3://"):
                        if not image.persistent_profile_path.endswith("/"):
                            image.persistent_profile_path = f"{image.persistent_profile_path}/"
                if not image_id:
        ...
        def getKasms(self, manager_id=None, operational_status=None, image_id=None, zone_id=None, exclude_zone=False, user_id='', staging_config_id=None, autoscale_config_id=None):
            if operational_status:
                if isinstanceoperational_statusstr:
                    operational_status = [
                     operational_status]
            session = self.session()
            try:
        ...
        def assignContainer(self, kasm_id, user_id, keepalive_expiration, start_date, cast_config_id=None):
            ...
            try:
                ...
                session.query(Kasm).filter(Kasm.kasm_id == kasm_id).filter(Kasm.user_id == None).filter(Kasm.is_standby == True).filter(or_(Kasm.operational_status == SESSION_OPERATIONAL_STATUS.RUNNING.value)(Kasm.operational_status == SESSION_OPERATIONAL_STATUS.STARTING.value)).update({'user_id':user_id, 
                 'is_standby':False, 
                 'start_date':(datetime.datetime.utcnow)(), 
                 'expiration_date':_expiration_date, 
                 'cast_config_id':cast_config_id})
                session.commit()
        ...
        def getExpiredKasms(self, zone_id=None, manager_id=None, operational_status=None):
            if not operational_status:
                operational_status = [SESSION_OPERATIONAL_STATUS.RUNNING.value,
                 SESSION_OPERATIONAL_STATUS.STOPPED.value,
                 SESSION_OPERATIONAL_STATUS.STOPPING.value,
                 SESSION_OPERATIONAL_STATUS.PAUSED.value,
                 SESSION_OPERATIONAL_STATUS.PAUSING.value,
        ...
        def createServerParse error at or near `DUP_TOP' instruction at offset 82

        def create_serverParse error at or near `DUP_TOP' instruction at offset 164

        def update_server(self, server, server_type=None, enabled=None, hostname=None, connection_type=None, connection_info=None, max_simultaneous_sessions=None, zone_id=None, server_pool_id=None, cores_override=None, memory_override=None, gpus_override=None, prune_images_mode=None, friendly_name=None, connection_username=None, connection_password=None, connection_port=None, connection_private_key=None, connection_passphrase=None, use_user_private_key=None, instance_id=None, autoscale_config_id=None, provider=None, container_limit=None, reusable=None, agent_installed=None, operational_status=None):
            attributes = [
             "server_type","enabled","hostname","connection_type","connection_info",
             "max_simultaneous_sessions", "zone_id", "server_pool_id", "cores_override", "memory_override",
             "gpus_override", "prune_images_mode", "friendly_name", "connection_username", "reusable",
             "connection_password", "connection_port", "connection_private_key", "container_limit",
             "connection_passphrase", "use_user_private_key", "instance_id", "autoscale_config_id", "provider",
             "agent_installed", "operational_status"]
            for x in attributes:
                val = locals()[x]
                if val != None:
                    if x in Server._sanitize:
                        if not not is_sanitized(val):
        ...
        def filter_client_settings_by_connection_type(self, client_settings: dict, connection_type: str) -> dict:
        ...
        def get_server_side_client_settings(self, client_settings):
            response = {'KASM_SVC_AUDIO':1 if (client_settings.get("allow_kasm_audio")) else 0, 
             'KASM_SVC_UPLOADS':1 if (client_settings.get("allow_kasm_uploads")) else 0, 
             'KASM_SVC_SEND_CUT_TEXT':"-SendCutText %s" % (1 if client_settings.get("allow_kasm_clipboard_down") else 0), 
             'KASM_SVC_ACCEPT_CUT_TEXT':"-AcceptCutText %s" % (1 if client_settings.get("allow_kasm_clipboard_up") else 0), 
             'KASM_SVC_AUDIO_INPUT':1 if (client_settings.get("allow_kasm_microphone")) else 0, 
             'KASM_SVC_DOWNLOADS':1 if (client_settings.get("allow_kasm_downloads")) else 0, 
             'KASM_SVC_GAMEPAD':1 if (client_settings.get("allow_kasm_gamepad")) else 0, 
             'KASM_SVC_WEBCAM':1 if (client_settings.get("allow_kasm_webcam")) else 0, 
             'KASM_SVC_PRINTER':1 if (client_settings.get("allow_kasm_printing")) else 0}
            return response
        ...
        def create_ldap_config(self, name, enabled, url, auto_create_app_user, search_base, search_filter, group_membership_filter, email_attribute=None, search_subtree=True, service_account_dn=None, service_account_password=None, connection_timeout=5, ldap_id=None, username_domain_match=None):
            session = self.session()
            ldap_config = LDAPConfig(ldap_id=ldap_id, name=name,
              enabled=enabled,
              url=url,
              auto_create_app_user=auto_create_app_user,
              ...
              username_domain_match=username_domain_match)
            try:
        ...
        def set_saml_config(self, strict, debug, auto_login, enabled, is_default, hostname, sp_entity_id, sp_acs_url, sp_slo_url, want_attribute_statement, name_id_encrypted, authn_request_signed, logout_request_signed, logout_response_signed, sign_metadata, want_messages_signed, want_assertions_signed, want_name_id, want_name_id_encrypted, want_assertions_encrypted, signature_algorithm, digest_algorithm, group_attribute=None, display_name=None, sp_name_id=None, idp_entity_id=None, idp_sso_url=None, idp_slo_url=None, idp_x509_cert=None, sp_x509_cert=None, sp_private_key=None, adfs=False, logo_url=None, saml_id=None):
            session = self.session()
            saml_config = SAMLConfig(saml_id=saml_id, strict=strict,
              debug=debug,
        ...
        def update_saml_config(self, saml_config, strict, is_default, hostname, debug, auto_login, sp_entity_id, sp_acs_url, sp_slo_url, enabled, want_attribute_statement, name_id_encrypted, authn_request_signed, logout_request_signed, logout_response_signed, sign_metadata, want_messages_signed, want_assertions_signed, want_name_id, want_name_id_encrypted, want_assertions_encrypted, signature_algorithm, digest_algorithm, group_attribute=None, display_name=None, sp_name_id=None, idp_entity_id=None, idp_sso_url=None, idp_slo_url=None, idp_x509_cert=None, sp_x509_cert=None, sp_private_key=None, adfs=False, logo_url=None):
            attributes = [
             "strict","debug","is_default","hostname","display_name","auto_login","group_attribute","sp_entity_id","sp_acs_url",
             "sp_slo_url", "sp_name_id", "idp_entity_id", "idp_sso_url", "idp_slo_url", "idp_x509_cert",
             "sp_x509_cert", "sp_private_key", "adfs", "enabled", "want_attribute_statement",
             "name_id_encrypted", "authn_request_signed", "logout_request_signed", "logout_response_signed",
             "sign_metadata", "want_messages_signed", "want_assertions_signed", "want_name_id",
             "want_name_id_encrypted", "want_assertions_encrypted", "signature_algorithm", "digest_algorithm",
             "logo_url"]
            for x in attributes:
                val = locals()[x]
                if val != None:
                    if x in SAMLConfig._sanitize:
                        if not not is_sanitized(val):
        ...
        def update_url_filter_policy(self, url_filter_policy, filter_policy_name=None, filter_policy_descriptions=None, categories=None, domain_blacklist=False, domain_whitelist=False, deny_by_default=None, enable_categorization=None, ssl_bypass_domains=None, ssl_bypass_ips=None, enable_safe_search=None, safe_search_patterns=None, disable_logging=False):
            attributes = [
             "filter_policy_name","filter_policy_descriptions","categories","domain_blacklist",
             "domain_whitelist", "deny_by_default", "enable_categorization", "ssl_bypass_domains",
             "ssl_bypass_ips", "enable_safe_search", "safe_search_patterns", "disable_logging"]
            for x in attributes:
                val = locals()[x]
                if val != None:
        ...
        def add_domains(self, categories, domains, is_system, ignore_duplicate=True, requested=0):
            ...
            for x in range(0, len(domains), step):
                objects = []
                for y in rangex(x + step):
                    if num_domains - 1 >= y:
                        _d = Domain(categories=categories,
                          domain_name=(domains[y]),
                          is_system=is_system,
                          requested=requested,
        ...
        def createGroup(self, group=None, name=None, description=None, priority=4095, group_metadata=None):
            if group is None:
                group = Group()
                group.name = name
        ...
        def update_group(self, group, name=None, description=None, priority=4095, group_metadata=None):
            attributes = [
             "name", "description", "priority", "group_metadata"]
            for x in attributes:
                val = locals()[x]
                if val != None:
                    setattr(group, x, val)
        ...
        def updateGroupMapping(self, group_mapping, sso_group_attributes=None, apply_to_all_users=False):
            attributes = [
             "sso_group_attributes", "apply_to_all_users"]
            for x in attributes:
                val = locals()[x]
                if val is not None:
        ...
        def process_key(self, license_key):
            ...
            return (
             license_key, decoded_license_data)
        ...
        def createManager(self, manager_id, manager_version, manager_hostname, zone_id):
            ...
            manager = Manager(manager_id=manager_id,
              ...
              first_reported=(datetime.datetime.utcnow()),
        ...
        def updateZone(self, zone=None, zone_name=None, allow_origin_domain=None, upstream_auth_address=None, proxy_connections=None, proxy_hostname=None, proxy_path=None, proxy_port=None, load_strategy=None, prioritize_static_agents=None, search_alternate_zones=None):
            attributes = [
             "zone_name","allow_origin_domain","upstream_auth_address","proxy_connections","proxy_hostname","proxy_path",
             "proxy_port", "load_strategy", "prioritize_static_agents", "search_alternate_zones"]
            for x in attributes:
                val = locals()[x]
                if val is not None:
        ...
        def update_autoscale_last_provisionParse error at or near `BEGIN_FINALLY' instruction at offset 92

        def update_autoscale_request_downscale_atParse error at or near `BEGIN_FINALLY' instruction at offset 86
        ...
        def getGroupAccountsSummary(self, group_id, start_date, end_date, user_ids):
            ...
            try:
                act_s_date = case([
                 (
                  Accounting.start_date < start_date, start_date)],
                  else_=(Accounting.start_date))
                act_e_date = case([
                 (
                  Accounting.destroyed_date > end_date, end_date)],
                  else_=(Accounting.destroyed_date))
                ...
                kasm_s_date = case([
                 (
                  Kasm.start_date < start_date, start_date)],
                  else_=(Kasm.start_date))
        ...
        def getAccountings(self, cast_config_id=None, user_ip=None, after=None, server_id=None, kasm_ids=None, page=None, page_size=None, filters=None, or_filters=None, sort_by='', sort_direction='desc'):
            ...
            try:
                ...
                for filter_ in filters:
                    ...
                    if not value:
                        pass
                    elif not name == "username" or name == "user_name":
                ...
                for filter_ in or_filters:
                    ...
                    if not name == "username" or name == "user_name":
                ...
                if or_filters:
                    ...
                    sort_column = getattr(Accounting, sort_by if sort_by is not None else "", "")
                    if sort_column:
                        sort_column = sort_column.desc() if sort_direction == "desc" else sort_column.asc()
                        q = q.order_by(nullslast(sort_column))
                    if page is not None:
        ...
        def createAccounting(self, kasm_id, user_id, user_name, image_id, image_name, image_src, image_friendly_name, start_date, created_date, destroyed_date, destroy_reason, group_ids, usage_hours, user_ip, cast_config_id, staging_config_id, zone_id, zone_name, server_id, server_hostname, docker_network, is_queued):
            account = Accounting(kasm_id=kasm_id,
              user_id=user_id,
              user_name=user_name,
              image_id=image_id,
              image_name=image_name,
              image_src=image_src,
              image_friendly_name=image_friendly_name,
              ...
              user_ip=user_ip,
              cast_config_id=cast_config_id,
              staging_config_id=staging_config_id,
              zone_id=zone_id,
              zone_name=zone_name,
              server_id=server_id,
        ...
        def updateAccounting(self, accounting, user_id=None, user_name=None, start_date=None, destroyed_date=None, destroy_reason=None, usage_hours=None, user_ip=None, cast_config_id=None, server_id=None, server_hostname=None, is_queued=None):
            attributes = [
             "user_id","user_name","start_date","destroyed_date","destroy_reason",
             "usage_hours", "user_ip", "cast_config_id", "server_id", "server_hostname", "is_queued"]
            for x in attributes:
                val = locals()[x]
                if val != None:
        ...
        def addSessionRecording(self, account_id, session_recording_url, session_recording_metadata):
            recording = SessionRecording(account_id=account_id, session_recording_url=session_recording_url,
              session_recording_metadata=session_recording_metadata)
            session = self.session()
            try:
        ...
        def updateSessionRecording(self, session_recording, session_recording_url=None, session_recording_metadata=None):
            attributes = [
             "session_recording_url", "session_recording_metadata"]
            for x in attributes:
                val = locals()[x]
                if val is not None:
        ...
        def validateApiKeyParse error at or near `BEGIN_FINALLY' instruction at offset 172
        ...
        def create_branding_config(self, name, favicon_logo_url, header_logo_url, html_title, login_caption, login_logo_url, loading_session_text, joining_session_text, destroying_session_text, is_default, hostname, login_splash_url, launcher_background_url):
            ...
            try:
                if is_default:
                    ...
                    branding_config = BrandingConfig(name=name,
                      favicon_logo_url=favicon_logo_url,
                      header_logo_url=header_logo_url,
                      html_title=html_title,
                      login_caption=login_caption,
                      login_logo_url=login_logo_url,
                      login_splash_url=login_splash_url,
                      loading_session_text=loading_session_text,
                      joining_session_text=joining_session_text,
                      destroying_session_text=destroying_session_text,
                      is_default=is_default,
                      hostname=hostname,
                      launcher_background_url=launcher_background_url)
                    session.add(branding_config)
                    session.commit()
        ...
        def update_branding_config(self, branding_config, name=None, favicon_logo_url=None, header_logo_url=None, html_title=None, login_caption=None, login_logo_url=None, loading_session_text=None, joining_session_text=None, destroying_session_text=None, is_default=None, hostname=None, login_splash_url=None, launcher_background_url=None):
            attributes = [
             "name","favicon_logo_url","header_logo_url","html_title","login_caption","login_logo_url",
             "loading_session_text", "joining_session_text", "destroying_session_text", "is_default",
             "hostname", "login_splash_url", "launcher_background_url"]
            for x in attributes:
                val = locals()[x]
                if val != None:
        ...
        def hasFilterWithCategorizationParse error at or near `LOAD_CONST' instruction at offset 44
        ...
        def create_staging_config(self, zone_id, server_pool_id, autoscale_config_id, image_id, num_sessions, expiration, allow_kasm_audio, allow_kasm_uploads, allow_kasm_downloads, allow_kasm_clipboard_down, allow_kasm_clipboard_up, allow_kasm_microphone, allow_kasm_gamepad, allow_kasm_webcam, allow_kasm_printing):
            session = self.session()
            try:
                staging_config = StagingConfig(zone_id=zone_id,
                  server_pool_id=server_pool_id,
                  autoscale_config_id=autoscale_config_id,
                  image_id=image_id,
                  num_sessions=num_sessions,
                  expiration=expiration,
                  allow_kasm_audio=allow_kasm_audio,
                  allow_kasm_uploads=allow_kasm_uploads,
                  allow_kasm_downloads=allow_kasm_downloads,
                  allow_kasm_clipboard_down=allow_kasm_clipboard_down,
                  allow_kasm_clipboard_up=allow_kasm_clipboard_up,
                  allow_kasm_microphone=allow_kasm_microphone,
                  allow_kasm_gamepad=allow_kasm_gamepad,
                  allow_kasm_webcam=allow_kasm_webcam,
        ...
        def update_staging_config(self, staging_config, zone_id=None, server_pool_id=None, autoscale_config_id=None, image_id=None, num_sessions=None, expiration=None, allow_kasm_audio=None, allow_kasm_uploads=None, allow_kasm_downloads=None, allow_kasm_clipboard_down=None, allow_kasm_clipboard_up=None, allow_kasm_microphone=None, allow_kasm_gamepad=None, allow_kasm_webcam=None, allow_kasm_printing=None):
            attributes = [
             "zone_id","image_id","num_sessions","expiration","allow_kasm_audio","allow_kasm_uploads",
             "allow_kasm_downloads", "allow_kasm_clipboard_down", "allow_kasm_clipboard_up",
             "allow_kasm_microphone", "allow_kasm_gamepad", "allow_kasm_webcam", "allow_kasm_printing"]
            for x in attributes:
                val = locals()[x]
                if val != None:
        ...
        def create_cast_config(self, image_id, allowed_referrers, limit_sessions, session_remaining, limit_ips, ip_request_limit, ip_request_seconds, error_url, enable_sharing, disable_control_panel, disable_tips, disable_fixed_res, key, allow_anonymous, group_id, require_recaptcha, kasm_url, dynamic_kasm_url, dynamic_docker_network, allow_resume, enforce_client_settings, allow_kasm_audio, allow_kasm_uploads, allow_kasm_downloads, allow_kasm_clipboard_down, allow_kasm_clipboard_up, allow_kasm_microphone, allow_kasm_sharing, kasm_audio_default_on, kasm_ime_mode_default_on, allow_kasm_gamepad, allow_kasm_webcam, allow_kasm_printing, valid_until, casting_config_name, remote_app_configs):
            ...
            session = self.session()
            try:
                cast_config = CastConfig(image_id=image_id,
                  allowed_referrers=allowed_referrers,
                  limit_sessions=limit_sessions,
                  session_remaining=session_remaining,
                  limit_ips=limit_ips,
                  ip_request_limit=ip_request_limit,
                  ip_request_seconds=ip_request_seconds,
                  error_url=error_url,
                  enable_sharing=enable_sharing,
                  disable_control_panel=disable_control_panel,
                  disable_tips=disable_tips,
                  disable_fixed_res=disable_fixed_res,
                  key=key,
                  allow_anonymous=allow_anonymous,
                  group_id=group_id,
                  require_recaptcha=require_recaptcha,
                  kasm_url=kasm_url,
                  dynamic_kasm_url=dynamic_kasm_url,
                  dynamic_docker_network=dynamic_docker_network,
                  allow_resume=allow_resume,
                  enforce_client_settings=enforce_client_settings,
                  allow_kasm_audio=allow_kasm_audio,
                  allow_kasm_uploads=allow_kasm_uploads,
                  allow_kasm_downloads=allow_kasm_downloads,
                  allow_kasm_clipboard_down=allow_kasm_clipboard_down,
                  allow_kasm_clipboard_up=allow_kasm_clipboard_up,
                  allow_kasm_microphone=allow_kasm_microphone,
                  allow_kasm_sharing=allow_kasm_sharing,
                  kasm_audio_default_on=kasm_audio_default_on,
                  kasm_ime_mode_default_on=kasm_ime_mode_default_on,
                  allow_kasm_gamepad=allow_kasm_gamepad,
                  allow_kasm_webcam=allow_kasm_webcam,
                  allow_kasm_printing=allow_kasm_printing,
                  valid_until=valid_until,
                  casting_config_name=casting_config_name,
        ...
        def decrement_cast_session_limitParse error at or near `BEGIN_FINALLY' instruction at offset 94
        ...
        def create_file_map(self, name, description, content, destination, file_type='text', is_readable=True, is_writable=False, is_executable=False, user_id=None, group_id=None, image_id=None, kasm_id=None):
            session = self.session()
            try:
                file_map = FileMap(created=(datetime.datetime.utcnow()),
                  user_id=user_id,
                  group_id=group_id,
                  image_id=image_id,
                  kasm_id=kasm_id,
        ...
        def update_file_map(self, file_map, name=None, description=None, content=None, destination=None, file_type=None, is_readable=None, is_writable=None, is_executable=None, user_id=None, group_id=None, image_id=None, kasm_id=None):
            attributes = [
             "name","description","content","destination","file_type","is_readable",
             "is_writable", "is_executable", "user_id",
             "group_id", "image_id", "kasm_id"]
            for x in attributes:
                val = locals()[x]
                if val is not None:
        ...
        def create_session_permission(self, kasm_id, user_id, access, vnc_username=None, vnc_password=None):
            session = self.session()
            try:
                session_permissions = SessionPermission(kasm_id=kasm_id,
                  user_id=user_id,
                  access=access,
                  vnc_username=(vnc_username if vnc_username else uuid.uuid4().hex[0:15]),
                  vnc_password=(vnc_password if vnc_password else uuid.uuid4().hex))
        ...
        def create_physical_tokens(self, tokens):
            ...
            try:
                for t in tokens:
                    token = PhysicalToken(serial_number=(t["serial_number"]),
                      seed_filename=(t["seed_filename"]),
                      token_seed=(t["token_seed"]),
                      created=(datetime.datetime.utcnow()))
                    session.add(token)
        ...
        def create_sso_attribute_mapping(self, attribute_name, user_field, oidc_id=None, saml_id=None, ldap_id=None):
            session = self.session()
            try:
                sso_attribute_mapping = SSOAttributeToUserFieldMapping(attribute_name=attribute_name,
        ...
        def create_physical_token(self, serial_number, seed_filename, token_seed, token_id=None):
            session = self.session()
            try:
                token = PhysicalToken(serial_number=serial_number,
        ...
        def create_oidc_config(self, auto_login, enabled, is_default, hostname, display_name, client_id, client_secret, auth_url, token_url, scope, redirect_url, user_info_url, logo_url, username_attribute, groups_attribute, debug, oidc_id=None):
            session = self.session()
            try:
                oidc_config = OIDCConfig(oidc_id=oidc_id,
                  auto_login=auto_login,
                  enabled=enabled,
                  is_default=is_default,
                  hostname=hostname,
        ...
        def update_oidc_config(self, oidc_config, auto_login=None, enabled=None, is_default=None, hostname=None, display_name=None, client_id=None, client_secret=None, auth_url=None, token_url=None, scope=None, redirect_url=None, user_info_url=None, logo_url=None, username_attribute=None, groups_attribute=None, debug=None):
            attributes = [
             "auto_login","enabled","is_default","hostname","display_name","client_id","client_secret",
             "auth_url", "token_url", "scope", "redirect_url", "user_info_url", "logo_url",
             "username_attribute", "groups_attribute", "debug"]
            for x in attributes:
                val = locals()[x]
                if val != None:
                    if x in OIDCConfig._sanitize:
                        if not not is_sanitized(val):
        ...
        def create_connection_proxy(self, server_address, server_port, connection_proxy_type, auth_token, zone_id, connection_proxy_id=None):
            session = self.session()
            try:
                connection_proxy = ConnectionProxy(connection_proxy_id=connection_proxy_id,
        ...
        def update_connection_proxy(self, connection_proxy, server_address=None, server_port=None, connection_proxy_type=None, auth_token=None, zone_id=None):
            attributes = [
             "server_address","server_port","connection_proxy_type","auth_token","zone_id"]
            for x in attributes:
                val = locals()[x]
                if val != None:
                    if x in ConnectionProxy._sanitize:
                        if not not is_sanitized(val):
        ...
        def create_server_pool(self, server_pool_name, server_pool_type, server_pool_id=None):
            session = self.session()
            try:
                server_pool = ServerPool(server_pool_id=server_pool_id,
                  server_pool_name=server_pool_name,
                  server_pool_type=server_pool_type)
        ...
        def update_server_pool(self, server_pool, server_pool_name=None, server_pool_type=None):
            attributes = [
             "server_pool_name", "server_pool_type"]
            for x in attributes:
                val = locals()[x]
                if val != None:
                    if x in ServerPool._sanitize:
                        if not not is_sanitized(val):
        ...
        def create_azure_config(self, config_name, max_instances, azure_subscription_id, azure_resource_group, azure_tenant_id, azure_client_id, azure_client_secret, azure_region, azure_vm_size, azure_os_disk_type, azure_image_reference, azure_network_sg, azure_subnet, azure_os_disk_size_gb, azure_tags, azure_os_username, azure_os_password, azure_ssh_public_key, startup_script, azure_config_override, azure_public_ip, azure_authority, azure_is_windows, azure_config_id=None):
            session = self.session()
            try:
                azure_config = AzureVMConfig(azure_config_id=azure_config_id,
                  ...
                  azure_os_disk_type=azure_os_disk_type,
                  azure_image_reference=azure_image_reference,
                  azure_network_sg=azure_network_sg,
                  azure_subnet=azure_subnet,
                  azure_os_disk_size_gb=azure_os_disk_size_gb,
                  azure_tags=azure_tags,
                  azure_os_username=azure_os_username,
                  azure_os_password=azure_os_password,
                  azure_ssh_public_key=azure_ssh_public_key,
                  startup_script=startup_script,
                  azure_config_override=azure_config_override,
                  azure_public_ip=azure_public_ip,
                  azure_authority=azure_authority,
                  azure_is_windows=azure_is_windows)
        ...
        def update_azure_config(self, azure_vm_config, config_name=None, max_instances=None, azure_subscription_id=None, azure_resource_group=None, azure_tenant_id=None, azure_client_id=None, azure_client_secret=None, azure_region=None, azure_vm_size=None, azure_os_disk_type=None, azure_image_reference=None, azure_network_sg=None, azure_subnet=None, azure_os_disk_size_gb=None, azure_tags=None, azure_os_username=None, azure_os_password=None, azure_ssh_public_key=None, startup_script=None, azure_config_override=None, azure_public_ip=None, azure_authority=None, azure_is_windows=None):
            attributes = [
             "config_name","max_instances","azure_subscription_id","azure_resource_group",
             "azure_tenant_id", "azure_client_id", "azure_client_secret", "azure_region", "azure_vm_size", "azure_os_disk_type",
             "azure_image_reference", "azure_network_sg", "azure_subnet", "azure_os_disk_size_gb",
             "azure_os_disk_size_gb", "azure_tags", "azure_os_username", "azure_os_password", "azure_ssh_public_key",
             "startup_script", "azure_config_override", "azure_public_ip", "azure_authority", "azure_is_windows"]
            for x in attributes:
                val = locals()[x]
                if val != None:
                    if x in AzureVMConfig._sanitize:
                        if not not is_sanitized(val):
        ...
        def create_aws_vm_config(self, config_name, max_instances, aws_ec2_instance_type, aws_region, aws_access_key_id, aws_secret_access_key, aws_ec2_ami_id, aws_ec2_public_key, aws_ec2_private_key, aws_ec2_security_group_ids, aws_ec2_subnet_id, startup_script, aws_ec2_iam, aws_ec2_ebs_volume_type, aws_ec2_ebs_volume_size_gb, aws_ec2_custom_tags, retrieve_password, aws_ec2_config_override, aws_config_id=None):
            session = self.session()
            try:
                aws_vm_config = AwsVMConfig(aws_config_id=aws_config_id,
                  config_name=config_name,
                  max_instances=max_instances,
                  ...
                  aws_ec2_public_key=aws_ec2_public_key,
                  ...
                  startup_script=startup_script,
                  aws_ec2_iam=aws_ec2_iam,
                  aws_ec2_ebs_volume_type=aws_ec2_ebs_volume_type,
                  aws_ec2_ebs_volume_size_gb=aws_ec2_ebs_volume_size_gb,
                  aws_ec2_custom_tags=aws_ec2_custom_tags,
                  retrieve_password=retrieve_password,
                  aws_ec2_config_override=aws_ec2_config_override)
        ...
        def update_aws_vm_config(self, aws_vm_config, config_name=None, max_instances=None, aws_ec2_instance_type=None, aws_region=None, aws_access_key_id=None, aws_secret_access_key=None, aws_ec2_ami_id=None, aws_ec2_public_key=None, aws_ec2_private_key=None, aws_ec2_security_group_ids=None, aws_ec2_subnet_id=None, startup_script=None, aws_ec2_iam=None, aws_ec2_ebs_volume_type=None, aws_ec2_ebs_volume_size_gb=None, aws_ec2_custom_tags=None, retrieve_password=None, aws_ec2_config_override=None):
            attributes = [
             "config_name","max_instances","aws_ec2_instance_type","aws_region","aws_access_key_id","aws_secret_access_key",
             "aws_ec2_ami_id", "aws_ec2_instance_type", "aws_ec2_public_key", "aws_ec2_private_key", "aws_ec2_security_group_ids", "aws_ec2_subnet_id",
             "startup_script", "aws_ec2_iam", "aws_ec2_ebs_volume_type", "aws_ec2_ebs_volume_size_gb", "aws_ec2_custom_tags", "retrieve_password",
             "aws_ec2_config_override"]
            for x in attributes:
                val = locals()[x]
                if val is not None:
                    if x in AwsVMConfig._sanitize:
                        if not not is_sanitized(val):
        ...
        def create_digital_ocean_vm_config(self, config_name, max_instances, digital_ocean_token, region, digital_ocean_droplet_image, digital_ocean_droplet_size, digital_ocean_tags, digital_ocean_sshkey_name, digital_ocean_firewall_name, startup_script, config_id=None):
            session = self.session()
            try:
                digital_ocean_vm_config = DigitalOceanVMConfig(config_id=config_id,
                  config_name=config_name,
                  max_instances=max_instances,
                  digital_ocean_token=digital_ocean_token,
                  region=region,
                  digital_ocean_droplet_image=digital_ocean_droplet_image,
                  digital_ocean_droplet_size=digital_ocean_droplet_size,
                  digital_ocean_tags=digital_ocean_tags,
                  digital_ocean_sshkey_name=digital_ocean_sshkey_name,
                  digital_ocean_firewall_name=digital_ocean_firewall_name,
                  startup_script=startup_script)
        ...
        def update_digital_ocean_vm_config(self, digital_ocean_vm_config, config_name=None, max_instances=None, digital_ocean_token=None, region=None, digital_ocean_droplet_image=None, digital_ocean_droplet_size=None, digital_ocean_tags=None, digital_ocean_sshkey_name=None, digital_ocean_firewall_name=None, startup_script=None):
            attributes = [
             "config_name","max_instances","digital_ocean_token","region","digital_ocean_droplet_image","digital_ocean_droplet_size",
             "digital_ocean_tags", "digital_ocean_sshkey_name", "digital_ocean_firewall_name", "startup_script"]
            for x in attributes:
                val = locals()[x]
                if val is not None:
                    if x in DigitalOceanVMConfig._sanitize:
                        if not not is_sanitized(val):
        ...
        def create_oci_vm_config(self, config_name, max_instances, oci_fingerprint, oci_tenancy_ocid, oci_region, oci_compartment_ocid, oci_availability_domains, oci_shape, oci_image_ocid, oci_subnet_ocid, oci_ssh_public_key, startup_script, oci_user_ocid, oci_private_key, oci_flex_cpus, oci_flex_memory_gb, oci_boot_volume_gb, oci_custom_tags, oci_config_override, oci_baseline_ocpu_utilization, oci_storage_vpus_per_gb, oci_nsg_ocids, config_id=None):
            session = self.session()
            try:
                oci_vm_config = OracleVMConfig(config_id=config_id,
                  config_name=config_name,
                  max_instances=max_instances,
                  ...
                  oci_flex_cpus=oci_flex_cpus,
                  oci_flex_memory_gb=oci_flex_memory_gb,
                  oci_boot_volume_gb=oci_boot_volume_gb,
                  oci_custom_tags=oci_custom_tags,
                  oci_config_override=oci_config_override,
                  oci_baseline_ocpu_utilization=oci_baseline_ocpu_utilization,
                  oci_storage_vpus_per_gb=oci_storage_vpus_per_gb,
                  oci_nsg_ocids=oci_nsg_ocids)
        ...
        def update_oci_vm_config(self, oci_vm_config, config_name=None, max_instances=None, oci_fingerprint=None, oci_tenancy_ocid=None, oci_region=None, oci_compartment_ocid=None, oci_availability_domains=None, oci_shape=None, oci_image_ocid=None, oci_subnet_ocid=None, oci_ssh_public_key=None, startup_script=None, oci_user_ocid=None, oci_private_key=None, oci_flex_cpus=None, oci_flex_memory_gb=None, oci_boot_volume_gb=None, oci_custom_tags=None, oci_config_override=None, oci_baseline_ocpu_utilization=None, oci_storage_vpus_per_gb=None, oci_nsg_ocids=None):
            attributes = [
             "config_name","max_instances","oci_fingerprint","oci_tenancy_ocid","oci_region","oci_compartment_ocid",
             "oci_availability_domains", "oci_shape", "oci_image_ocid", "oci_subnet_ocid", "oci_ssh_public_key", "startup_script",
             "oci_user_ocid", "oci_private_key", "oci_flex_cpus", "oci_flex_memory_gb", "oci_boot_volume_gb", "oci_custom_tags",
             "oci_config_override", "oci_baseline_ocpu_utilization", "oci_storage_vpus_per_gb", "oci_nsg_ocids"]
            for x in attributes:
                val = locals()[x]
                if val is not None:
                    if x in OracleVMConfig._sanitize:
                        if not not is_sanitized(val):
        ...
        def create_gcp_vm_config(self, config_name, max_instances, gcp_project, gcp_region, gcp_zone, gcp_machine_type, gcp_image, startup_script, gcp_boot_volume_gb, gcp_cmek, gcp_disk_type, gcp_network, gcp_subnetwork, gcp_public_ip, gcp_network_tags, gcp_custom_labels, gcp_credentials, gcp_metadata, gcp_service_account, gcp_guest_accelerators, gcp_config_override, config_id=None):
            session = self.session()
            try:
                gcp_vm_config = GcpVMConfig(config_id=config_id,
                  config_name=config_name,
                  max_instances=max_instances,
                  ...
                  gcp_image=gcp_image,
                  startup_script=startup_script,
                  gcp_boot_volume_gb=gcp_boot_volume_gb,
                  gcp_cmek=gcp_cmek,
                  gcp_disk_type=gcp_disk_type,
                  gcp_network=gcp_network,
                  gcp_subnetwork=gcp_subnetwork,
                  gcp_public_ip=gcp_public_ip,
                  gcp_network_tags=gcp_network_tags,
                  gcp_custom_labels=gcp_custom_labels,
                  gcp_credentials=gcp_credentials,
                  gcp_metadata=gcp_metadata,
                  gcp_service_account=gcp_service_account,
                  gcp_guest_accelerators=gcp_guest_accelerators,
        ...
        def update_gcp_vm_config(self, gcp_vm_config, config_name=None, max_instances=None, gcp_project=None, gcp_region=None, gcp_zone=None, gcp_machine_type=None, gcp_image=None, startup_script=None, gcp_boot_volume_gb=None, gcp_cmek=None, gcp_disk_type=None, gcp_network=None, gcp_subnetwork=None, gcp_public_ip=None, gcp_network_tags=None, gcp_custom_labels=None, gcp_credentials=None, gcp_metadata=None, gcp_service_account=None, gcp_guest_accelerators=None, gcp_config_override=None):
            attributes = [
             "config_name","max_instances","gcp_project","gcp_region","gcp_zone","gcp_machine_type","gcp_image","startup_script",
             "gcp_boot_volume_gb", "gcp_cmek", "gcp_disk_type", "gcp_network", "gcp_subnetwork", "gcp_public_ip", "gcp_network_tags",
             "gcp_custom_labels", "gcp_credentials", "gcp_metadata", "gcp_service_account", "gcp_guest_accelerators", "gcp_config_override"]
            for x in attributes:
                val = locals()[x]
                if val is not None:
                    if x in GcpVMConfig._sanitize:
                        if not not is_sanitized(val):
        ...
        def create_vsphere_vm_config(self, config_name, max_instances, vsphere_vcenter_address, vsphere_vcenter_port, vsphere_vcenter_username, vsphere_vcenter_password, vsphere_template_name, vsphere_datacenter_name, vsphere_vm_folder, vsphere_datastore, vsphere_cluster_name, vsphere_resource_pool, vsphere_datastore_cluster_name, startup_script, vsphere_os_username, vsphere_os_password, vsphere_cpus, vsphere_memoryMB, vsphere_installed_OS_type, config_id=None):
            session = self.session()
            try:
                vsphere_vm_config = VsphereVMConfig(config_id=config_id,
                  config_name=config_name,
                  max_instances=max_instances,
                  vsphere_vcenter_address=vsphere_vcenter_address,
                  vsphere_vcenter_port=vsphere_vcenter_port,
                  vsphere_vcenter_username=vsphere_vcenter_username,
                  vsphere_vcenter_password=vsphere_vcenter_password,
                  vsphere_template_name=vsphere_template_name,
                  vsphere_datacenter_name=vsphere_datacenter_name,
                  ...
                  vsphere_datastore_cluster_name=vsphere_datastore_cluster_name,
                  startup_script=startup_script,
                  vsphere_os_username=vsphere_os_username,
                  vsphere_os_password=vsphere_os_password,
                  vsphere_cpus=vsphere_cpus,
                  vsphere_memoryMB=vsphere_memoryMB,
                  vsphere_installed_OS_type=vsphere_installed_OS_type)
        ...
        def update_vsphere_vm_config(self, vsphere_vm_config, config_name=None, max_instances=None, vsphere_vcenter_address=None, vsphere_vcenter_port=None, vsphere_vcenter_username=None, vsphere_vcenter_password=None, vsphere_template_name=None, vsphere_datacenter_name=None, vsphere_vm_folder=None, vsphere_datastore=None, vsphere_cluster_name=None, vsphere_resource_pool=None, vsphere_datastore_cluster_name=None, startup_script=None, vsphere_os_username=None, vsphere_os_password=None, vsphere_cpus=None, vsphere_memoryMB=None, vsphere_installed_OS_type=None):
            attributes = [
             "config_name","max_instances","vsphere_vcenter_address","vsphere_vcenter_port","vsphere_vcenter_username",
             "vsphere_vcenter_password", "vsphere_template_name", "vsphere_datacenter_name", "vsphere_vm_folder", "vsphere_datastore",
             "vsphere_cluster_name", "vsphere_resource_pool", "vsphere_datastore_cluster_name", "startup_script", "vsphere_os_username",
             "vsphere_os_password", "vsphere_cpus", "vsphere_memoryMB", "vsphere_installed_OS_type"]
            for x in attributes:
                val = locals()[x]
                if val is not None:
                    if x in VsphereVMConfig._sanitize:
                        if not not is_sanitized(val):
        ...
        def create_openstack_vm_config(self, config_name, max_instances, openstack_keystone_endpoint, openstack_nova_endpoint, openstack_nova_version, openstack_glance_endpoint, openstack_glance_version, openstack_cinder_endpoint, openstack_cinder_version, openstack_project_name, openstack_project_domain_name, openstack_auth_method, openstack_user_domain_name, openstack_username, openstack_password, openstack_application_credential_id, openstack_application_credential_secret, openstack_metadata, openstack_image_id, openstack_flavor, openstack_create_volume, openstack_volume_size_gb, openstack_volume_type, startup_script, openstack_security_groups, openstack_network_id, openstack_key_name, openstack_availability_zone, openstack_config_override, config_id=None):
            session = self.session()
            try:
                openstack_vm_config = OpenStackVMConfig(config_id=config_id,
                  config_name=config_name,
                  max_instances=max_instances,
                  openstack_keystone_endpoint=openstack_keystone_endpoint,
                  openstack_nova_endpoint=openstack_nova_endpoint,
                  openstack_nova_version=openstack_nova_version,
                  openstack_glance_endpoint=openstack_glance_endpoint,
                  openstack_glance_version=openstack_glance_version,
                  openstack_cinder_endpoint=openstack_cinder_endpoint,
                  openstack_cinder_version=openstack_cinder_version,
                  ...
                  openstack_auth_method=openstack_auth_method,
                  openstack_username=openstack_username,
                  openstack_password=openstack_password,
                  openstack_application_credential_id=openstack_application_credential_id,
                  openstack_application_credential_secret=openstack_application_credential_secret,
                  openstack_image_id=openstack_image_id,
                  openstack_metadata=openstack_metadata,
                  openstack_flavor=openstack_flavor,
                  openstack_create_volume=openstack_create_volume,
                  ...
                  openstack_key_name=openstack_key_name,
                  openstack_availability_zone=openstack_availability_zone,
                  openstack_config_override=openstack_config_override)
        ...
        def update_openstack_vm_config(self, openstack_vm_config, config_name=None, max_instances=None, openstack_keystone_endpoint=None, openstack_nova_endpoint=None, openstack_nova_version=None, openstack_glance_endpoint=None, openstack_glance_version=None, openstack_cinder_endpoint=None, openstack_cinder_version=None, openstack_project_name=None, openstack_project_domain_name=None, openstack_user_domain_name=None, openstack_auth_method=None, openstack_username=None, openstack_password=None, openstack_application_credential_id=None, openstack_application_credential_secret=None, openstack_image_id=None, openstack_metadata=None, openstack_flavor=None, openstack_create_volume=None, openstack_volume_size_gb=None, openstack_volume_type=None, openstack_security_groups=None, startup_script=None, openstack_network_id=None, openstack_key_name=None, openstack_availability_zone=None, openstack_config_override=None):
            attributes = [
             "config_name","max_instances","openstack_keystone_endpoint","openstack_nova_endpoint","openstack_nova_version",
             "openstack_glance_endpoint", "openstack_glance_version", "openstack_cinder_endpoint", "openstack_cinder_version", "openstack_project_name",
             "openstack_project_domain_name", "openstack_user_domain_name", "openstack_auth_method", "openstack_username", "openstack_password",
             "openstack_application_credential_id", "openstack_application_credential_secret", "openstack_image_id", "openstack_metadata",
             "openstack_flavor", "openstack_create_volume", "openstack_volume_size_gb", "openstack_volume_type", "openstack_security_groups",
             "startup_script", "openstack_network_id", "openstack_key_name", "openstack_availability_zone", "openstack_config_override"]
            for x in attributes:
                val = locals()[x]
                if val is not None:
                    if x in OpenStackVMConfig._sanitize:
                        if not not is_sanitized(val):
        ...
        def create_azure_dns_config(self, config_name, azure_subscription_id, azure_resource_group, azure_tenant_id, azure_client_id, azure_client_secret, azure_region, azure_authority, azure_dns_config_id=None):
            session = self.session()
            try:
                azure_dns_config = AzureDNSConfig(azure_dns_config_id=azure_dns_config_id,
                  config_name=config_name,
                  azure_subscription_id=azure_subscription_id,
                  azure_resource_group=azure_resource_group,
                  azure_tenant_id=azure_tenant_id,
        ...
        def update_azure_dns_config(self, azure_dns_config, config_name=None, azure_subscription_id=None, azure_resource_group=None, azure_tenant_id=None, azure_client_id=None, azure_client_secret=None, azure_region=None, azure_authority=None):
            attributes = [
             "config_name","azure_subscription_id","azure_resource_group",
             "azure_tenant_id", "azure_client_id", "azure_client_secret", "azure_region", "azure_authority"]
            for x in attributes:
                val = locals()[x]
                if val != None:
        ...
        def create_aws_dns_config(self, config_name, aws_access_key_id, aws_secret_access_key, config_id=None):
            session = self.session()
            try:
                aws_dns_config = AwsDNSConfig(config_id=config_id,
                  config_name=config_name,
                  aws_access_key_id=aws_access_key_id,
                  aws_secret_access_key=aws_secret_access_key)
        ...
        def update_aws_dns_config(self, aws_dns_config, config_name=None, aws_access_key_id=None, aws_secret_access_key=None):
            attributes = [
             "config_name", "aws_access_key_id", "aws_secret_access_key"]
            for x in attributes:
                val = locals()[x]
                if val is not None:
                    if x in AwsDNSConfig._sanitize:
                        if not not is_sanitized(val):
        ...
        def create_digital_ocean_dns_config(self, config_name, digital_ocean_token, config_id=None):
            session = self.session()
            try:
                digital_ocean_dns_config = DigitalOceanDNSConfig(config_id=config_id,
                  config_name=config_name,
                  digital_ocean_token=digital_ocean_token)
        ...
        def update_digital_ocean_dns_config(self, digital_ocean_dns_config, config_name=None, digital_ocean_token=None):
            attributes = [
             "config_name", "digital_ocean_token"]
            for x in attributes:
                val = locals()[x]
                if val is not None:
                    if x in DigitalOceanDNSConfig._sanitize:
                        if not not is_sanitized(val):
        ...
        def create_oci_dns_config(self, config_name, oci_user_ocid, oci_private_key, oci_fingerprint, oci_tenancy_ocid, oci_region, oci_compartment_ocid, config_id=None):
            session = self.session()
            try:
                oci_dns_config = OracleDNSConfig(config_id=config_id,
                  config_name=config_name,
                  oci_user_ocid=oci_user_ocid,
                  oci_private_key=oci_private_key,
                  oci_fingerprint=oci_fingerprint,
        ...
        def update_oci_dns_config(self, oci_dns_config, config_name=None, oci_user_ocid=None, oci_private_key=None, oci_fingerprint=None, oci_tenancy_ocid=None, oci_region=None, oci_compartment_ocid=None):
            attributes = [
             "config_name","oci_user_ocid","oci_private_key","oci_fingerprint","oci_tenancy_ocid","oci_region","oci_compartment_ocid"]
            for x in attributes:
                val = locals()[x]
                if val is not None:
        ...
        def create_gcp_dns_config(self, config_name, gcp_credentials, gcp_project, config_id=None):
            session = self.session()
            try:
                gcp_dns_config = GcpDNSConfig(config_id=config_id,
                  config_name=config_name,
                  gcp_credentials=gcp_credentials,
                  gcp_project=gcp_project)
        ...
        def update_gcp_dns_config(self, gcp_dns_config, config_name=None, gcp_credentials=None, gcp_project=None):
            attributes = [
             "config_name", "gcp_credentials", "gcp_project"]
            for x in attributes:
                val = locals()[x]
                if val is not None:
        ...
        def get_gcp_dns_config(self, config_id):
            session = self.session()
            try:
        ...
        def delete_vm_provider_config(self, vm_provider_config):
            self._delete(vm_provider_config)

        def delete_dns_provider_config(self, dns_provider_config):
            self._delete(dns_provider_config)

        def get_autoscale_configs(self, server_pool_id=None, zone_id=None, enabled=None, autoscale_type=None):
            session = self.session()
            try:
        ...
        def get_autoscale_config(self, autoscale_config_id):
            session = self.session()
            try:
        ...
        def create_autoscale_config(self, autoscale_config_name, autoscale_type, enabled, standby_cores, standby_memory_mb, standby_gpus, downscale_backoff, register_dns, base_domain_name, nginx_cert, nginx_key, agent_cores_override, agent_memory_override_gb, agent_gpus_override, aws_config_id, aws_dns_config_id, azure_config_id, azure_dns_config_id, zone_id, server_pool_id, connection_type, connection_info, connection_port, connection_username, connection_password, reusable, hooks, minimum_pool_standby_sessions, max_simultaneous_sessions_per_server, ldap_id, ad_create_machine_record, ad_computer_container_dn, ad_recursive_machine_record_cleanup, digital_ocean_vm_config_id, digital_ocean_dns_config_id, openstack_vm_config_id, oci_vm_config_id, oci_dns_config_id, gcp_vm_config_id, gcp_dns_config_id, vsphere_vm_config_id, connection_private_key, use_user_private_key, connection_passphrase, require_checkin, aggressive_scaling, autoscale_config_id=None, agent_installed=False):
            session = self.session()
            try:
                if connection_type == CONNECTION_TYPE.KASMVNC.value:
                    if max_simultaneous_sessions_per_server:
                        if int(max_simultaneous_sessions_per_server) > 1:
                            max_simultaneous_sessions_per_server = 1
                    autoscale_config = AutoScaleConfig(autoscale_config_id=autoscale_config_id,
                      autoscale_config_name=autoscale_config_name,
                      autoscale_type=autoscale_type,
                      enabled=enabled,
                      standby_cores=standby_cores,
                      standby_memory_mb=standby_memory_mb,
                      standby_gpus=standby_gpus,
                      downscale_backoff=downscale_backoff,
                      last_provision=(datetime.datetime.fromtimestamp(0)),
                      request_downscale_at=(datetime.datetime.fromtimestamp(0)),
                      register_dns=register_dns,
                      base_domain_name=base_domain_name,
                      nginx_cert=nginx_cert,
                      nginx_key=nginx_key,
                      agent_cores_override=agent_cores_override,
                      agent_memory_override_gb=agent_memory_override_gb,
                      agent_gpus_override=agent_gpus_override,
                      aws_config_id=aws_config_id,
                      aws_dns_config_id=aws_dns_config_id,
                      azure_config_id=azure_config_id,
                      azure_dns_config_id=azure_dns_config_id,
                      digital_ocean_vm_config_id=digital_ocean_vm_config_id,
                      digital_ocean_dns_config_id=digital_ocean_dns_config_id,
                      openstack_vm_config_id=openstack_vm_config_id,
                      oci_vm_config_id=oci_vm_config_id,
                      oci_dns_config_id=oci_dns_config_id,
                      gcp_vm_config_id=gcp_vm_config_id,
                      gcp_dns_config_id=gcp_dns_config_id,
                      vsphere_vm_config_id=vsphere_vm_config_id,
                      zone_id=zone_id,
                      server_pool_id=server_pool_id,
                      ...
                      connection_private_key=connection_private_key,
                      use_user_private_key=use_user_private_key,
                      connection_passphrase=connection_passphrase,
                      reusable=reusable,
                      hooks=hooks,
                      minimum_pool_standby_sessions=minimum_pool_standby_sessions,
                      max_simultaneous_sessions_per_server=max_simultaneous_sessions_per_server,
                      ldap_id=ldap_id,
                      ad_create_machine_record=ad_create_machine_record,
                      ad_recursive_machine_record_cleanup=ad_recursive_machine_record_cleanup,
                      ad_computer_container_dn=ad_computer_container_dn,
        ...
        def update_autoscale_config(self, autocale_config, enabled=None, autoscale_config_name=None, autoscale_type=None, standby_cores=None, standby_memory_mb=None, standby_gpus=None, downscale_backoff=None, register_dns=None, base_domain_name=None, nginx_cert=None, nginx_key=None, agent_cores_override=None, agent_memory_override_gb=None, agent_gpus_override=None, connection_type=None, connection_info=None, connection_port=None, connection_username=None, connection_password=None, reusable=None, hooks=None, minimum_pool_standby_sessions=None, max_simultaneous_sessions_per_server=None, azure_config_id=None, azure_dns_config_id=None, aws_config_id=None, server_pool_id=None, zone_id=None, ldap_id=None, ad_create_machine_record=None, ad_recursive_machine_record_cleanup=None, ad_computer_container_dn=None, aws_dns_config_id=None, digital_ocean_vm_config_id=None, digital_ocean_dns_config_id=None, oci_vm_config_id=None, oci_dns_config_id=None, gcp_vm_config_id=None, gcp_dns_config_id=None, vsphere_vm_config_id=None, connection_private_key=None, use_user_private_key=None, connection_passphrase=None, agent_installed=None, require_checkin=None, openstack_vm_config_id=None, aggressive_scaling=None):
            attributes = [
             "enabled","autoscale_config_name","autoscale_type","standby_cores","standby_memory_mb",
             "standby_gpus", "downscale_backoff", "register_dns", "base_domain_name", "nginx_cert", "nginx_key",
             "agent_cores_override", "agent_memory_override_gb", "agent_gpus_override", "connection_type",
             "connection_info", "connection_port", "connection_username", "connection_password", "reusable",
             "hooks", "minimum_pool_standby_sessions", "max_simultaneous_sessions_per_server", "zone_id",
             "ldap_id", "ad_create_machine_record", "ad_recursive_machine_record_cleanup",
             "ad_computer_container_dn", "connection_password", "connection_private_key",
             "connection_passphrase", "use_user_private_key", "agent_installed", "require_checkin",
             "aggressive_scaling"]
            for x in attributes:
                val = locals()[x]
                if val is not None:
                    if x in AutoScaleConfig._sanitize:
                        if not not is_sanitized(val):
        ...
        def delete_autoscale_config(self, autoscale_config):
            self._delete(autoscale_config)

        def create_schedule(self, autoscale_config_id: str, days_of_the_week: typing.List, active_start_time: datetime.time, active_end_time: datetime.time, timezone: str):
            session = self.session()
            ZoneInfo(timezone)
            schedule = Schedule(autoscale_config_id=autoscale_config_id,
        ...
        def update_schedule(self, schedule: Schedule, days_of_the_week: typing.List=None, active_start_time: datetime.time=None, active_end_time: datetime.time=None, timezone: str=None):
            attributes = [
             "days_of_the_week", "active_start_time", "active_end_time", "timezone"]
            if timezone:
                ZoneInfo(timezone)
            for x in attributes:
                val = locals()[x]
                if val is not None:
                    if x in Schedule._sanitize:
                        if not not is_sanitized(val):
        ...
        def create_storage_mapping(self, name, storage_provider_id, enabled, config=None, user_id=None, image_id=None, group_id=None, target=None, read_only=None, s3_access_key_id=None, s3_bucket=None, s3_secret_access_key=None, webdav_user=None, webdav_pass=None, oauth_token=None):
            session = self.session()
            storage_mapping = StorageMapping(name=name,
        ...
        def get_storage_mapping(self, storage_mapping_id=None, user_id=None, group_id=None, image_id=None):
            session = self.session()
            try:
                q = session.query(StorageMapping)
                if storage_mapping_id:
        ...
        def update_storage_mapping(self, storage_mapping, name=None, storage_provider_id=None, config=None, enabled=None, user_id=None, group_id=None, image_id=None, target=None, read_only=None, s3_access_key_id=None, s3_secret_access_key=None, s3_bucket=None, webdav_user=None, webdav_pass=None, oauth_token=None):
            attributes = [
             "name","storage_provider_id","config","enabled","user_id","group_id","image_id",
             "target", "read_only", "s3_access_key_id", "s3_secret_access_key", "s3_bucket", "webdav_user",
             "webdav_pass", "oauth_token"]
            for x in attributes:
                val = locals()[x]
                if val is not None:
                    if x in StorageMapping._sanitize:
                        if not not is_sanitized(val):
        ...
        def delete_storage_mapping(self, storage_mapping):
        ...
        def get_storage_providers(self, storage_provider_id=None, storage_provider_type=None, enabled=None):
            session = self.session()
            try:
                q = session.query(StorageProvider)
                if storage_provider_id:
        ...
        def create_storage_provider(self, name, storage_provider_type, client_id, client_secret, auth_url, token_url, webdav_url, scope, redirect_url, auth_url_options, volume_config, mount_config, root_drive_url, default_target, enabled):
            session = self.session()
            storage_provider = StorageProvider(name=name,
              ...
              scope=scope,
              redirect_url=redirect_url,
              auth_url_options=auth_url_options,
              volume_config=volume_config,
              mount_config=mount_config,
              root_drive_url=root_drive_url,
        ...
        def update_storage_provider(self, storage_provider, name=None, storage_provider_type=None, client_id=None, client_secret=None, auth_url=None, token_url=None, webdav_url=None, scope=None, redirect_url=None, auth_url_options=None, volume_config=None, mount_config=None, root_drive_url=None, default_target=None, enabled=None):
            attributes = [
             "name","storage_provider_type","client_id","client_secret","auth_url","token_url",
             "webdav_url", "scope", "redirect_url", "auth_url_options", "volume_config", "mount_config",
             "root_drive_url", "default_target", "enabled"]
            for x in attributes:
                val = locals()[x]
                if val is not None:
        ...
        def delete_storage_provider(self, storage_provider):
        ...
        def delete_group_permission(self, group_permission):
            self._delete(group_permission)

        def get_group_permission(self, group_permission_id):
            session = self.session()
            try:
        ...
        def create_registry(self, config, schema_version=None, do_auto_update=False):
            session = self.session()
            try:
        ...
        def update_registry(self, registry_id, update):
            session = self.session()
            try:
        ...
        def get_registry_by_url(self, registry_url):
            session = self.session()
            try:
        ...
        def create_webauthn_credential(self, user_id, authenticator_credential_id, public_key, sign_count=None, webauthn_credential_id=None):
            session = self.session()
            try:
                credential = WebauthnCredential(created=(datetime.datetime.utcnow()),
                  user_id=user_id,
        ...
        def delete_webauthn_credentials(self, user_id):
            session = self.session()
            try:
        ...
        def create_webauthn_request(self, request_id, challenge):
            session = self.session()
            try:
                request = WebauthnRequest(webauthn_request_id=request_id,
                  challenge=challenge,
                  created=(datetime.datetime.utcnow()))
                session.add(request)
                session.commit()
        ...
        def consume_webauthn_request(self, request_id):
            session = self.session()
        ...
        def delete_expired_webauthn_requests(self):
            session = self.session()
            max_request_life = int(self.config["auth"]["webauthn_request_lifetime"].value)
